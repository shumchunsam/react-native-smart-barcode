...
public class RCTCaptureModule extends ReactContextBaseJavaModule {
    ...
    // //将消息msg发送给RN侧
    // mContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit("AndroidToRNMessage",
    // msg);
    //
    // }
        ...
        if (captureManager.cap != null) {
            getCurrentActivity().runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    // Toast.makeText(getCurrentActivity(), "startFlash",
    ...
    public void DecodeFromPath(final String path, final Callback errorCallback, final Callback successCallback) {
        ...
        // Toast.makeText(getCurrentActivity(), "DecodeFromPath:"+path,
        // Toast.LENGTH_SHORT).show();
    ...
    public void playBeep() {
        if (captureManager.cap != null) {
            getCurrentActivity().runOnUiThread(new Runnable() {
                public void run() {
                    captureManager.cap.playBeepSoundAndVibrate();
                }
            });
        }
    }
    ...
    /*
     * @ReactMethod public void changeWidthHeight(final int width,final int height)
     * {
     * 
     * if (captureManager.cap != null) { activity.runOnUiThread(new Runnable() {
     * public void run() { captureManager.cap.setCHANGE_WIDTH(width, height); } });
     * } }
     */
}
